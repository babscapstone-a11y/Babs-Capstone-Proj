...
<head>
    ...
    <title>@yield('title', 'Cashier') – BAB'S RESTO</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .cb-brand { display: flex; align-items: center; gap: .75rem; }
        .logo-badge {
        ...
        .cb-brand-text { color: var(--white); font-weight: 700; font-size: 1rem; line-height: 1.2; }
        .cb-brand-sub  { color: rgba(255,255,255,0.4); font-size: .72rem; margin-top: .1rem; }

        .cb-topbar-right { display: flex; align-items: center; gap: 1rem; }
        .cb-nav { display: flex; align-items: center; gap: .5rem; }
        .nav-link-cb {
        ...
        .cb-clock { font-size: 1.05rem; font-weight: 700; font-variant-numeric: tabular-nums; }
        .cb-staff { display: flex; align-items: center; gap: .6rem; }
        .cb-staff-avatar {
        ...
        .cb-staff-name { color: var(--white); font-size: .85rem; font-weight: 600; }
        .cb-staff-role { color: rgba(255,255,255,0.4); font-size: .68rem; }
        .btn-logout-cb {
        ...
        .btn-logout-cb:hover { background: rgba(220,38,38,0.32); color: var(--white); }

        /* ── Page content ────────────────────────────────────── */
        .cb-content { padding: 1.5rem 1.75rem 2.5rem; }
        ...
        @media (max-width: 768px) {
            .cb-topbar { padding: .75rem 1rem; flex-wrap: wrap; gap: .6rem; }
...
<body>
    ...
    <header class="cb-topbar">
        <div class="cb-brand">
            ...
            <div>
                <div class="cb-brand-text">BAB'S RESTO</div>
                <div class="cb-brand-sub">Payment &amp; Billing</div>
            </div>
        </div>
        ...
        <div class="cb-topbar-right">
            ...
            <div class="cb-datetime">
                <div class="cb-date">{{ now()->format('l, F d, Y') }}</div>
                <div class="cb-clock" id="liveClock">--:--:-- --</div>
            </div>
            <div class="cb-staff">
                <div class="cb-staff-avatar">{{ auth()->user()->initials }}</div>
                <div>
                    <div class="cb-staff-name">{{ auth()->user()->name ?: auth()->user()->email }}</div>
                    <div class="cb-staff-role">Cashier</div>
                </div>
            </div>
            <form method="POST" action="{{ route('logout') }}" id="logoutForm">
                @csrf
            </form>
            <button type="button" class="btn-logout-cb" onclick="openConfirmModal({
                    title: 'Log Out?',
